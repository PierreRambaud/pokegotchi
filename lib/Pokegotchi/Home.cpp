...
static void start_button_event_handler(lv_event_t* e) {
  ...
  serial_printf("HOME", "%d", res);
  if(res != LV_FS_RES_OK) serial_printf("HOME", "ERROR opening directory");

  char fn[256];
  while(1) {
      res = lv_fs_dir_read(&dir, fn, sizeof(fn));
      if(res != LV_FS_RES_OK) {
          serial_printf("HOME", "ERROR reading directory");
          break;
      }
      ...
      serial_printf("HOME", "%s\n", fn);
  }

  lv_fs_dir_close(&dir);
  serial_printf("HOME", "Close directory\n");
